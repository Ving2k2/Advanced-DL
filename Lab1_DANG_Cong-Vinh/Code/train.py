...
# Initializes directory for checkpoints
...
for epoch in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        ...
        ############## Task 5
        batch_idx = idx[i:min(i + batch_size, n_train)]

        x_batch = torch.LongTensor(X_train[batch_idx]).to(device)
...
}, os.path.join(save_dir, 'model_deepsets.pth.tar'))
...
torch.save({
    ...
    'optimizer' : optimizer.state_dict(),
}, os.path.join(save_dir, 'model_lstm.pth.tar'))
